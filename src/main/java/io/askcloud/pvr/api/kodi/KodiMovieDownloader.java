...
public class KodiMovieDownloader extends KodiDownloader {
	...
	public KodiMovieDownloader(String name,String imdbID,String status,int percentDownloaded,String downloadFile,int downloadSize, int totalSize) {
		super(imdbID,null,name,status,percentDownloaded,downloadFile,downloadSize,totalSize);
		LOG.entering(CLASS_NAME, "KodiMovieDownloader",new Object[]{name,imdbID,percentDownloaded,downloadFile});
		LOG.exiting(CLASS_NAME, "KodiMovieDownloader");
	}
	...
	protected void requestDownload() {
		...
		String movie = URLEncoder.encode(getName());
		...
		HTPC.getInstance().getConMgr().call(exodus, new ApiCallback<ListModel.FileItem>() {
			...
			public void onResponse(AbstractCall<ListModel.FileItem> call) {
				...
				if(movieURL != null)
				{
					LOG.fine("Movie: " + movieURL);
					downloadMovieFromURL(getName(),movieURL);
				}
				else
				{
					LOG.severe("ERROR: Unable to find Movie named: " + getName());
				}
			}
		...
		try {
			HTPC.getInstance().getConMgr().call(exodus, new ApiCallback<String>() {
				...
				public void onResponse(AbstractCall<String> call) {
					LOG.entering(CLASS_NAME, "onResponse::onResponse", new Object[] { call });
					
					LOG.exiting(CLASS_NAME, "clearCacheAndSources::onResponse");
				}

				@Override
				public void onError(int code, String message, String hint) {
					LOG.entering(CLASS_NAME, "onResponse::onResponse", new Object[] { code,message,hint });
					LOG.severe("ERROR " + code + ": " + message);
					
					LOG.exiting(CLASS_NAME, "onError::onResponse");
				}
			});
		}
		catch (Exception e) {
			e.printStackTrace();
			LOG.severe("Exception from Kodi on downloading the moive.  Exception: " + e.getMessage());
		}
